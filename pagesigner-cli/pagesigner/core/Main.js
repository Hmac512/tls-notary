...
export class Main {
  constructor() {
    ...
    if (typeof window != "undefined") {
      this.is_chrome = window.navigator.userAgent.match("Chrome")
        ? true
        : false;
      this.is_firefox = window.navigator.userAgent.match("Firefox")
        ? true
        : false;
  ...
  async main() {
    ...
    const that = this;
    this.messageListener = chrome.runtime.onMessage.addListener(function (
      data
    ) {
    ...
    console.log(this.trustedOracle, this.trustedOracle.IP);
    ...
    // notary is online
    const URLFetcherDoc = await getURLFetcherDoc(
      globals.defaultNotaryIP,
      globals.defaultNotaryPort
    );
  ...
  async queryNotaryNoSandbox(IP) {
    ...
    const resp = await fetch(
      "http://" + IP + ":" + globals.defaultNotaryPort + "/getPubKey",
      {
        method: "POST",
        mode: "cors",
        cache: "no-store",
  ...
  async pingNotary(IP) {
    ...
    const fProm = fetch(
      "http://" + IP + ":" + globals.defaultNotaryPort + "/ping",
      {
        mode: "no-cors",
      }
    );
  ...
  async prepareNotarization(after_click = false, isPreview = false) {
    ...
    await new Promise(function (resolve) {
      ...
      chrome.webRequest.onSendHeaders.addListener(oSH_handler, {
        urls: ["<all_urls>"],
        tabId: active_tab.id,
        types: ["main_frame", "xmlhttprequest"],
      });
      ...
      .catch((err) => {
        ...
        if (typeof err === "object") {
          ...
          if (err.message == "Failed to fetch") {
            errmsg =
              "Unable to connect to the notary server. Please check your internet connection.";
          }
        }
        this.sendAlert({
          title: "PageSigner error.",
  ...
  async processMessages(data) {
    ...
    switch (data.message) {
      ...
      case "export":
        this.sendToManager(
          {
            pgsg: JSON.stringify(await this.getPGSG(data.sid)),
            name: (await getSession(data.sid)).sessionName,
          },
          "export"
        );
        break;
  ...
  async verifyPgsgV6(obj) {
    ...
    const notaryPubKey = await (
      await fetch("http://localhost:10011/getPubKey")
    ).text();
    console.log(notaryPubkey);
    ...
    if (!globals.useNotaryNoSandbox) {
      ...
      // Fetch the notary's pubkey from the notary server
    ...
    const vepsRV = await TLS.verifyECParamsSig(
      certPath[0],
      serverEcPubkey,
      rsaSig,
      cr,
    ...
    const pms = int2ba(
      (ba2int(obj["notary PMS share"]) + ba2int(obj["client PMS share"])) %
        P256prime,
      32
    );
    ...
    const commitHash = await TLSNotarySession.computeCommitHash(
      obj["server response records"]
    );
    const keyShareHash = await sha256(
      concatTA(clientCwkShare, clientCivShare, clientSwkShare, clientSivShare)
    );
    ...
    for (let i = 0; i < blockCount; i++) {
      ghashInputs.push(
        obj["client request ciphertext"].slice(i * 16, (i + 1) * 16)
      );
    }
    ...
    for (let i = 0; i < ciphertext.length; i++) {
      ...
      const nonce = concatTA(
        civ,
        int2ba(explicitNonce, 8),
        int2ba(blockCounter, 4)
      );
    ...
    const response = ba2str(concatTA(...responseRecords), false);
