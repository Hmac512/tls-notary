...
export class TLSNotarySession {
  ...
  async start() {
    ...
    const [encCF, tagCF, vdCF] = await this.twopc.run(
      cr,
      sr,
      this.tls.getAllHandshakes(),
      pmsShare
    );
    // Finished (0x14) with length 12 (0x0c)
    this.tls.updateAllHandshakes(
      concatTA(new Uint8Array([0x14, 0x00, 0x00, 0x0c]), vdCF)
    );
    ...
    const encRequestBlocks = this.encryptRequest(
      this.request,
      encCountersForRequest
    );
    const gctrBlocks = await this.twopc.getGctrBlocks();
    if (this.pm) this.pm.update("last_stage", { current: 9, total: 10 });
    const [ghashOutputs, ghashInputsBlob] =
      await this.twopc.getTagFromPowersOfH(encRequestBlocks);
    await this.tls.buildAndSendRequest(
      gctrBlocks,
      ghashOutputs,
      encRequestBlocks
    );
    ...
    const keyShareHash = await sha256(
      concatTA(cwkShare, civShare, swkShare, sivShare)
    );
    const pmsShareHash = await sha256(pmsShare);
    const data5 = await this.twopc.send(
      "commitHash",
      concatTA(commitHash, keyShareHash, pmsShareHash)
    );
    this.twopc.destroy();
    ...
    const [eKey, eValidFrom, eValidUntil, eSigByMasterKey] =
      this.twopc.getEphemeralKey();
  ...
  encryptRequest(request, encCounterBlocks) {
    ...
    for (let i = 0; i < encCounterBlocks.length; i++) {
      if (i == encCounterBlocks.length - 1) {
        ...
        encRequestBlocks.push(
          xor(
            encCounterBlocks[i].slice(0, lastBlockLen),
            this.request.slice(i * 16)
          )
        );
      } else {
        encRequestBlocks.push(
          xor(encCounterBlocks[i], this.request.slice(i * 16, i * 16 + 16))
        );
      }
    }
