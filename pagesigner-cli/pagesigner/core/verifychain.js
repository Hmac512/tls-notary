...
export function checkCertSubjName(cert, serverName) {
  ...
  for (let nameInCert of allNames) {
    if (wildTest(nameInCert, serverName) == true) return true;
  }
...
export async function verifyChain(chain_der, date, trustedCerts) {
  ...
  async function do_verify(chain, date, trustedCerts) {
    ...
    var pubkeyToFind = new Uint8Array(
      chain.slice(
        -1
      )[0].subjectPublicKeyInfo.subjectPublicKey.valueBlock.valueHex
    );
    ...
    for (const certificate of rv.certificatePath) {
