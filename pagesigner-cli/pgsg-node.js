...
const TLSNotarySession = (await import("./pagesigner/core/TLSNotarySession.js"))
  .TLSNotarySession;
...
global.chrome = {
  extension: { getURL: (url) => Path.join(__dirname, "pagesigner", url) },
};
...
function showUsage() {
  ...
  console.log(
    "Verify a Pagesigner session from imported.pgsg. This will create a session directory with the decrypted cleartext and a copy of the pgsg file.\r\n"
  );
  console.log("\r\n");
  process.exit(0);
}
  ...
  if (argv[2] === "notarize") {
    ...
    for (let i = 0; i < lines.length; i++) {
      if (blankLineWasFound) {
        ...
        // replace whatever delimiter is at the end with /r/n
